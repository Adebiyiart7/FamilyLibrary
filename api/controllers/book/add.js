...
const add = async (req, res) => {
  ...
  const { title, author, image, summary, tags, fullText } = req.body;
  ...
  // update update number of books count of each tag
  for (let tag of tagsArray) {
    tag = tag.trim().toLowerCase();

    updateTagBooksCount = await Tag.findOne({ name: tag });
    updateTagBooksCount.numberOfBooks += 1;
    await updateTagBooksCount.save();
  }
